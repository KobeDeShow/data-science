...
def fix_turnstile_data(filenames):
    ...
    for name in filenames:
        with open(name, "rb") as fin:
            with open("updated_" + name, "wb") as fout:
                reader = csv.reader(fin)
                writer = csv.writer(fout)
                for row in reader:
                    rows2write = [] # the rows to write
                    head = row[:3]  # the initial part of every row
                    
                    # extract multiple lines from row
                    for i in xrange(3, len(row), 5):
                        rows2write.append(head + row[i:i+5])
